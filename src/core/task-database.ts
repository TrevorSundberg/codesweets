 ...
 * I think all should be possible, but if we create an interface we then put
 * a chooser in its place instead
 ...
 * Easiest possible interface right now is to make
 * it so we can add the abstract class anyways
 * Not great, but it will make it so we can
